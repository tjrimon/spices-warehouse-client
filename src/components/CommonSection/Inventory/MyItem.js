    ...
    return (
        <div className='py-20 container mx-auto'>
            <div className=' pb-14 flex justify-around text-center items-center'>
                <h3 className='text-center text-3xl  uppercase font-semibold'>My Spices Inventory</h3>
                <button className='block  hover:bg-indigo-500 bg-orange-500 text-white py-3 px-6 rounded-full'><Link to='/addinventory'><FontAwesomeIcon icon="fa-solid fa-plus" /> Add Spices</Link></button>
            </div>
            <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
                {loading ? <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    {inventoryItems.map(item => <> {item.email == user.email ? <tbody key={item._id}>
                        <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4">
                                <div className='flex items-center'>
                                    ...
                                    <button onClick={() => handleDelete(item._id)} title='delete' className='text-indigo-500 hover:text-orange-500 text-lg'><FontAwesomeIcon icon="fa-solid fa-trash" /></button>
                                </div>
                            </td>
                        </tr>
                    </tbody> : ''}
